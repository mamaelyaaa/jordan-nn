...
class JordanNetwork:
    ...
    def __init__(
        ...
        self._init_weights()
        self._context: np.ndarray = np.zeros(out_layer.neurons)
    ...
    def _forward(self, training: np.ndarray) -> np.ndarray:
        """
        ...
        assert h_with_fictive.shape == (self._h_layer.neurons + 1,)
    ...
    def _backward(
        ...
        assert delta.shape == (self._o_layer.neurons,)

        # Рассчитываем значения невязок нейронов выходного слоя
        # P.S. r - residual (с англ. "невязка")
        r_y: np.ndarray = delta * self._o_layer.activation.derivative(self.__s_y)
        assert r_y.shape == (self._o_layer.neurons,)

        # Рассчитываем значения невязок нейронов скрытого слоя
        r_h: np.ndarray = np.dot(
            self.w_ho[:, :-1].T, r_y
        ) * self._h_layer.activation.derivative(self.__s_h)
        assert r_h.shape == (self._h_layer.neurons,)

        # Рассчитываются поправки весовых коэффициентов
        self._update_weights(self.__x_with_fictive, residual_out=r_y, residual_hid=r_h)
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            for i in range(len(x)):
                y_exp: np.ndarray = self._forward(training=x[i])
                total_error += self._backward(x[i], y[i], y_exp)
    ...
    def _update_weights(
        ...
        self.w_ho += self._lr * np.outer(residual_out, self.__h_with_fictive)
