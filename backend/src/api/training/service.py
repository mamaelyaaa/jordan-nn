...
from rnn.structure.regularizer import L1, L2
...
class TrainingServiceImpl:
    ...
    async def run_training(self, session_id: str, config: dict[str, Any]):
        if session_id in self.active_tasks:
            await self._send_error(session_id, "Обучение уже началось")
            return

        task = asyncio.create_task(self._training_flow(session_id, config))
        self.active_tasks[session_id] = task
        task.add_done_callback(lambda _: self.active_tasks.pop(session_id, None))

        # ================= FLOW =================

    async def _training_flow(self, session_id: str, config: dict):
        try:
            await self._update_status(session_id, "configuring")
            await self._send_ws(session_id, "training_started")

            dataset = await self._prepare_data(config)
            ...
            mse_history = await asyncio.get_running_loop().run_in_executor(
                self.executor,
                self._train_sync,
                session_id,
                config,
            ...
            if not mse_history:
                raise RuntimeError("Нет результатов обучения модели")

            predictions = await asyncio.get_running_loop().run_in_executor(
                self.executor, rnn_manager.predict
            )

            await self._complete(session_id, mse_history, predictions)

        except asyncio.CancelledError:
            await self._update_status(session_id, "stopped")
            await self._send_ws(session_id, "training_stopped")
            raise

        except Exception as e:
            await self._update_status(session_id, "error", error=str(e))
            await self._send_error(session_id, str(e))

    # ================= STEPS =================

    async def _prepare_data(self, config):
    ...
    @staticmethod
    def _prepare_data_sync(symbol: str, test_rate: float):
        rnn_manager.load_data(symbol)
        return rnn_manager.prepare_data(test_rate=test_rate)

    @staticmethod
    def _train_sync(session_id: str, config: dict, dataset: Dataset):
        return rnn_manager.model.train_with_control(
    ...
    async def _complete(self, session_id: str, mse, predictions):
        session = await session_manager.get_session(session_id)
        if not session:
            return

        session.predictions = PredictionsSchema(
            train_prediction=list(predictions.train_predictions),
            test_prediction=list(predictions.test_predictions),
        )

        await self._update_status(
            session_id,
            "completed",
            current_epoch=len(mse),
            loss_history=mse,
        ...
        await websocket_manager.send_progress(
            session_id,
            {
                ...
                "final_loss": mse[-1],
                "mse_history": mse,
                "predictions": session.predictions.model_dump(),
            },
        )
    ...
    @staticmethod
    def _configure_model(config: dict):
        activation_map = {
            "tanh": TanhActivation(),
        ...
        if "pct_return" not in features:
            features.insert(0, "pct_return")

        regularizer = None
        if config.get("regularizer"):
            regularizer = (
                L1(config["regularizer_rate"])
                if config["regularizer"] == "L1"
                else L2(config["regularizer_rate"])
            )
    ...
    @staticmethod
    async def _update_status(session_id: str, status: str, **extra):
        await session_manager.update_session(session_id, status=status, **extra)

    @staticmethod
    async def _send_ws(session_id: str, msg_type: str):
        await websocket_manager.send_progress(
            session_id, {"type": msg_type, "session_id": session_id}
        )

    @staticmethod
    async def _send_error(session_id: str, error: str):
        await websocket_manager.send_progress(
            session_id,
            {
                "type": "training_error",
...
#                         "type": "training_error",
#                         "session_id": session_id,
#                         "error": "Обучение для этой сессии уже началось",
#                     },
#                 )
#                 return
#
...
#             training_task = asyncio.create_task(self._train_async(session_id, config))
...
#
#         except Exception as e:
#             print(f"Ошибка старта обучения для сессии: {session_id}: {e}")
#             await self._handle_training_error(session_id, e)
#
#     async def _train_async(self, session_id: str, config: Dict[str, Any]):
...
#             await self._complete_training(session_id, mse_history)
#
#         except asyncio.CancelledError:
#             print(f"Обучения отменено для сессии: {session_id}")
#             await self._handle_training_cancelled(session_id)
#             raise
#
#         except Exception as e:
#             print(f"Ошибка обучения в сессии: {session_id}: {e}")
#             await self._handle_training_error(session_id, e)
#             raise
#
...
#             hidden_activation=activation_map[config["hidden_activation"]],
#             hidden_neurons=config["hidden_neurons"],
#             regularization=regularizer_config,
#         )
#
...
#
#         except Exception as e:
#             print(f"Ошибка обучения модели для сессии: {session_id}: {e}")
#             self._send_error_to_websocket(session_id, str(e))
#             return []
#
#     async def _complete_training(self, session_id: str, mse_history: List[float]):
#         """Завершение обучения и получение результатов"""
#         if not mse_history:
#             print(f"Нет истории обучения для сессии: {session_id}")
...
#         except Exception as e:
#             print(f"Ошибка отправки вебсокета: {e}")
#
...
training_service = TrainingServiceImpl()
