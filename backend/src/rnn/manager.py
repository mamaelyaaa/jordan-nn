...
class RNNManager:
    ...
    def __init__(
        self,
        target: Optional[TargetEnum | str] = None,
        features: Optional[list[FeaturesEnum | str]] = None,
        learning_rate: float = 0.003,
        hidden_activation: Optional[ActivationProtocol] = None,
        hidden_neurons: int = 128,
        regularization: Optional[RegularizerProtocol] = None,
    ...
    def load_data(self, stock_symbol: str, limit: int = 365) -> None:
        """Подгрузка данных"""

        file_path = f"{settings.files.stocks / stock_symbol.lower()}.us.txt"

        raw_data = self.loader.load_raw_data(source=file_path)
        data = raw_data.tail(limit)
        self.raw_data = data
        return

    def prepare_data(self, test_rate: float = 0.3) -> "Dataset":
        """Подготовка данных и разделение выборки"""

        if self.raw_data is None:
            raise ValueError("Нет сырых данных")
    ...
    def initialize_model(self) -> None:
        ...
        self.model = JordanRNN(
            hidden_layer=HiddenLayer(
                neurons=self.hidden_neurons, activation=self.hidden_activation
            ),
            output_layer=OutputLayer(activation=LinearActivation()),
    ...
    def set_config(
        ...
        """
        Обновление конфигурации модели после инициализации.

        Parameters:
        -----------
        features : Optional[list[FeaturesEnum]]
            Список признаков для использования
        learning_rate : Optional[float]
            Скорость обучения
        hidden_activation : Optional[ActivationProtocol]
            Функция активации скрытого слоя
        hidden_neurons : Optional[int]
            Количество нейронов в скрытом слое
        regularization : Optional[RegularizerProtocol]
            Регуляризатор
        target : Optional[TargetEnum]
            Целевая переменная
        """

        # Обновляем только если передано значение
        if features is not None:
            self.features = features
        ...
        # Если изменились параметры модели, нужно переинициализировать модель
        if any([learning_rate, hidden_activation, hidden_neurons, regularization]):
